...
const WORKER_SYSTEM_PROMPT =
  'You are a helpful career assistant for Pakistani skilled workers on the MazdoorPing platform. Provide advice in the same language the user writes in (Urdu or English). Keep responses practical and concise. Topics include: profile improvement, bidding tips, skill development, pricing strategies, workplace safety, and career growth. Be encouraging and supportive.';

export default function WorkerAIAssistantPage() {
      ...
      <div className="glass-card p-6">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-white/40">{t('aiChat.subtitle')}</p>
          </div>
          <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20">
            <Sparkles className="w-3.5 h-3.5 text-emerald-400" />
            <span className="text-xs font-medium text-emerald-400">{t('aiChat.poweredByAI')}</span>
          </div>
        </div>
      </div>
              ...
              {msg.role === 'assistant' ? (
                ...
                <div className="flex justify-end">
                  <div className="max-w-[85%] sm:max-w-[70%]">
                    <div className="px-4 py-3 rounded-2xl rounded-tr-md bg-emerald-600/15 border border-emerald-500/15 backdrop-blur-sm">
        ...
        {messages.length <= 1 && !isThinking && (
          <div className="px-4 sm:px-6 pb-3">
            <div className="flex items-center gap-2 mb-3">
              <Lightbulb className="w-4 h-4 text-emerald-400/50" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {suggestions.map((sug, i) => (
                <button
                  ...
                  className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-white/[0.03] border border-white/[0.06] hover:bg-emerald-500/10 hover:border-emerald-500/20 transition-all text-start group"
                >
                  <span className="text-xs text-white/40 group-hover:text-emerald-400 transition-colors">
                    {i < 3 ? (
                  ...
                  <ChevronRight className="w-3 h-3 text-white/20 group-hover:text-emerald-400 transition-colors shrink-0" />
                </button>
              ))}
            </div>
          </div>
